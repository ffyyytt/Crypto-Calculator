...
void md5::setState(int position, uint64 value)
{
    state[position] = value;
}

void md5::setBuffer(int position, uint8 value)
{
    buffer[position] = value;
}

uint64 md5::getState(int position)
{
    return state[position];
}

uint8 md5::getBuffer(int position)
{
...
void md5::transform(uint8 data[64])
{
    uint32 X[16], A, B, C, D;
...
void md5::update(uint8* input, uint32 length)
{
    uint32 left, fill;
...
char* Hash::bytesToHex(uint8* str, int n)
{
